  ...
  live_loop :kick do
    ...
    3.times do
      pattern.length.times do
        sample :bd_haus, rate: 0.5 ,  amp: (pattern[look].to_f / 9) if (pattern[tick] != "-")
        sleep 8/pattern.length.to_f
      end
    end
    tick_reset
    pattern_b.length.times do
      sample :bd_haus, rate: 0.5 ,  amp: (pattern_b[look].to_f / 9) if (pattern_b[tick] != "-")
      sleep 8/pattern_b.length.to_f
    end
    tick_reset
  end
end
...
live_loop :a do
  use_synth :prophet
  n = ring(45, 56, 62, 55).tick
  play n, amp: 0.15, release: rrand(0.3, 0.8), sustain: rrand(0.2, 1.2), attack: rrand(0.1, 0.5)
  sleep 1
end
